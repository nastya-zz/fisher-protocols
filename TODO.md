...
- Убрать `user_id` из всех Request message (GetFeedRequest, CreatePostRequest и т.д.)
- Gateway извлекает user_id из JWT и добавляет в gRPC metadata
- Сервисы читают user_id только из metadata

**Затронутые файлы:**

- `feed_v1/feed.proto` - убрать user_id из GetFeedRequest, GetLocationFeedRequest и др.
- `post_v1/post.proto` - убрать из CreatePostRequest, AddLikeRequest, AddCommentRequest
- `user_v1/user.proto` - убрать где дублируется
...
- [ ] #1: Убрать user_id из параметров
- [ ] #2: Кэширование валидации токенов
